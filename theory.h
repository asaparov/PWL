		...
		typename ProofCalculus::Language** formulas)
{
	...
	if (!array_map_init(c.types, ((size_t) 1) << (core::log2(type_count == 0 ? 1 : type_count) + 1))) {
		return false;
	} else if (!array_map_init(c.negated_types, ((size_t) 1) << (core::log2(negated_type_count == 0 ? 1 : negated_type_count) + 1))) {
		free(c.types); return false;
	} else if (!array_map_init(c.relations, ((size_t) 1) << (core::log2(relation_count == 0 ? 1 : relation_count) + 1))) {
		free(c.negated_types);
		free(c.types); return false;
	} else if (!array_map_init(c.negated_relations, ((size_t) 1) << (core::log2(negated_relation_count == 0 ? 1 : negated_relation_count) + 1))) {
		free(c.relations); free(c.negated_types);
		free(c.types); return false;
	} else if (!array_init(c.definitions, ((size_t) 1) << (core::log2(definition_count == 0 ? 1 : definition_count) + 1))) {
		free(c.relations); free(c.negated_types);
		free(c.types); free(c.negated_relations);
		return false;
	} else if (!array_init(c.existential_intro_nodes, ((size_t) 1) << (core::log2(existential_intro_node_count == 0 ? 1 : existential_intro_node_count) + 1))) {
		free(c.relations); free(c.negated_types);
		free(c.types); free(c.negated_relations);
		free(c.definitions); return false;
	} else if (!array_map_init(c.function_values, ((size_t) 1) << (core::log2(function_value_count == 0 ? 1 : function_value_count) + 1))) {
		free(c.relations); free(c.negated_types);
		free(c.types); free(c.negated_relations);
		free(c.definitions); free(c.existential_intro_nodes);
		return false;
...
core::free(*expected_conclusion); if (expected_conclusion->reference_count == 0) core::free(expected_conclusion);
	...
	inline bool are_elements_provable() {
		bool success = true;
	...
	inline bool add_unary_atom(Term& atom, Proof* axiom, Args&&... visitor)
	{
		...
		if (atom.binary.right->type == TermType::CONSTANT)
			try_init_concept(atom.binary.right->constant);
...
private:
	...
	static inline bool unify_subformula(
			...
			array<array_map<Formula*, Term*>>& unifications)
	{
		if (!array_map_init(unifications[0], 4)) {
			return false;
		...
		for (unsigned int i = 1; i < sets.set_count + 1; i++) {
			...
			for (unsigned int l = 0; l < set_formula_conjuncts.length; l++) {
				...
				for (unsigned int j = 0; j < possible_values.length; j++) {
					...
					if (!new_possible_values.ensure_capacity(new_possible_values.length + provable_elements.length + 1)) {
			...
			Prover& prover) const
	{
		...
		if (!(formula->type == FormulaType::EXISTS || (formula->type == FormulaType::UNARY_APPLICATION && formula->binary.left->type == FormulaType::VARIABLE)))
			return true;
				...
				if (AddSelfToAtoms || current != set_id) {
					...
					if (set_formula->type == FormulaType::AND) {
						for (unsigned int i = 0; i < set_formula->array.length; i++) {
							if (!new_ancestor_terms.add(make_pair(set_formula->array.operands[i], provable_element_count == sets.sets[current].set_size)))
								return false;
						}
					} else if (!new_ancestor_terms.add(make_pair(set_formula, provable_element_count == sets.sets[current].set_size))) {
						return false;
					}
				}
			...
			for (unsigned int i = 1; i < sets.set_count + 1; i++) {
				...
				if (provable_elements.length == sets.sets[i].set_size && set_formula->type == FormulaType::AND) {
					...
					for (const variable_assignment& possible_value : possible_values) {
						...
						if (src_variables == nullptr || !new_atoms.ensure_capacity(new_atoms.length + set_formula->array.length)) {
	...
	bool check_set_membership_after_subtraction(array<Formula*>& old_atoms, unsigned int removed_set, Args&&... visitor)
	{
		...
		while (old_atom_index < old_atoms.length) {
			...
			for (unsigned int i = 1; i < sets.set_count + 1; i++) {
				...
				array<array_map<Formula*, Term*>> unifications(4);
				if (!unify_subformula<true>(next_old_atom, set_formula, quantifiers, unifications)) {
				...
				if (sets.sets[i].provable_elements.length == sets.sets[i].set_size && set_formula->type == FormulaType::AND) {
					...
					for (const array_map<Formula*, Term*>& unification : unifications) {
						unsigned int substitution_count = 0;
						Term** src_variables = (Term**) malloc(sizeof(Term*) * sets.sets[i].arity * 2);
						if (src_variables == nullptr || !old_atoms.ensure_capacity(old_atoms.length + set_formula->array.length)) {
							if (src_variables != nullptr) core::free(src_variables);
							for (auto& element : unifications) core::free(element);
							for (auto entry : old_elements) core::free(entry.value);
							return false;
						...
						for (unsigned int k = 0; k < set_formula->array.length; k++) {
							...
							bool atom_already_exists = false;
							for (unsigned int k = 0; !atom_already_exists && k < old_atoms.length; k++)
								if (*old_atoms[k] == *substituted) atom_already_exists = true;
							if (atom_already_exists) {
								core::free(*substituted); if (substituted->reference_count == 0) core::free(substituted);
							} else {
								old_atoms[old_atoms.length++] = substituted;
							}
						}
						for (unsigned int j = 0; j < substitution_count; j++) {
							core::free(*src_variables[j]); if (src_variables[j]->reference_count == 0) core::free(src_variables[j]);
...
private:
			...
			Term*& constant, unsigned int& new_constant, Args&&... args)
	{
		...
		if (!get_constants(*this, quantified, variable, constants, std::forward<Args>(args)...)) {
			core::free(*var); if (var->reference_count == 0) core::free(var);
			return nullptr;
		}
		...
		OnProofSampleFunction on_new_proof_sample)
{
	...
	array<const Formula*> arg1_of(4);
	array<const Formula*> arg2_of(4);
	if (!filter_constants_helper<false>(T, canonicalized->quantifier.operand, canonicalized->quantifier.variable, constants, new_name_events, arg1_of, arg2_of)) {
		free(*canonicalized); if (canonicalized->reference_count == 0) free(canonicalized);
		return false;
	}
